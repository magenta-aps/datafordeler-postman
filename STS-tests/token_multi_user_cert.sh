...
echo "----------------------------"
STS_URL=https://sts-cert.test.data.gl/get_token?on_behalf_of=testbruger
...
echo "$SAML_XML" | grep '<saml2:Attribute Name="https://data.gl/claims/on-behalf-of">' >/dev/null
echo "$SAML_XML" | grep ">testbruger</saml2:AttributeValue>" >/dev/null
